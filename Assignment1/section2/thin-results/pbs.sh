...
mpiexec -np 2 -ppn=1 -env I_MPI_DEBUG 5 -genv I_MPI_FABRICS=ofi -genv I_MPI_OFI_PROVIDER=tcp ./IMB-MPI1_intel PingPong -msglog 28 2>/dev/null | grep -v ^\# | grep -v '^$' | grep -v '^\[' | sed -r 's/  */,/g' | cut -d "," -f2- >>result_node_ofi_tcp_intel.csv


mpiexec -np 2 -ppn=1 -env I_MPI_DEBUG 5 -genv I_MPI_FABRICS=ofi -genv I_MPI_OFI_PROVIDER=mlx ./IMB-MPI1_intel PingPong -msglog 28 2>/dev/null | grep -v ^\# | grep -v '^$' | grep -v '^\[' | sed -r 's/  */,/g' | cut -d "," -f2- >>result_node_ofi_mlx_intel.csv


mpiexec -np 2 -ppn=1 -env I_MPI_DEBUG 5 -genv I_MPI_FABRICS=ofi -genv I_MPI_OFI_PROVIDER=sockets ./IMB-MPI1_intel PingPong -msglog 28 2>/dev/null | grep -v ^\# | grep -v '^$' | grep -v '^\[' | sed -r 's/  */,/g' | cut -d "," -f2- >>result_node_ofi_sockets_intel.csv
